...
test('Entomoscope @real-protocol', async ({ app, page, browserName }) => {
	...
	await expect(app.metadata.textbox('Date', 'date')).toHaveValue('2026-02-09');
	await expect(app.metadata.textbox('Date', 'time')).toHaveValue('13:18');
	...
	await expect(app.metadata.textbox('Modèle de Raspberry Pi')).toHaveValue(
		'Raspberry Pi 4 Model B Rev 1.5'
	);
	await expect(app.metadata.textbox('Numéro de série du Raspberry Pi')).toHaveValue(
		'10000000ede1b681'
	);
