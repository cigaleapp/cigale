...
- Name: correct results.zip >> has all the images
- Location: /home/runner/work/cigale/cigale/tests/import.spec.js:25:2

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: getByText('leaf.jpeg', { exact: true }) resolved to 3 elements:
    1) <h2 class="svelte-1uv3axg">leaf.jpeg</h2> aka locator('[id="\\30 00001"]').getByRole('heading', { name: 'leaf.jpeg' })
    2) <h2 class="svelte-1uv3axg">leaf.jpeg</h2> aka locator('#loading_0').getByRole('heading', { name: 'leaf.jpeg' })
    3) <h2 class="svelte-1uv3axg">leaf.jpeg</h2> aka locator('#loading_2').getByRole('heading', { name: 'leaf.jpeg' })

Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByText('leaf.jpeg', { exact: true })

    at /home/runner/work/cigale/cigale/tests/import.spec.js:28:62
```
...
```ts
   ...
   4 | import { chooseDefaultProtocol, readdirTreeSync, setSettings, toast } from './utils';
   5 |
   6 | /**
   7 |  * @param {import('@playwright/test').Page} page
   8 |  * @param {string} filepath
   9 |  */
   10 | async function setupImport(page, filepath) {
   11 | 	await setSettings({ page }, { showTechnicalMetadata: false });
   12 | 	await chooseDefaultProtocol(page);
   13 | 	// Import fixture zip
   14 | 	await expect(page.getByText(/Cliquer ou déposer/)).toBeVisible();
   15 | 	const fileInput = await page.$("input[type='file']");
   16 | 	await fileInput?.setInputFiles(path.join('./tests/fixtures/exports/', filepath));
   17 | 	await page.waitForTimeout(3000);
   18 | }
   19 |
   20 | test.describe('correct results.zip', () => {
   21 | 	test.beforeEach(async ({ page }) => {
   22 | 		setupImport(page, 'correct.zip');
   23 | 	});
   24 |
   25 | 	test('has all the images', async ({ page }) => {
   26 | 		await expect(page.getByText('lil-fella.jpeg', { exact: true })).toBeVisible();
   27 | 		await expect(page.getByText('cyan.jpeg', { exact: true })).toBeVisible();
>  28 | 		await expect(page.getByText('leaf.jpeg', { exact: true })).toBeVisible();
      | 		                                                           ^ Error: expect.toBeVisible: Error: strict mode violation: getByText('leaf.jpeg', { exact: true }) resolved to 3 elements:
   29 | 		await expect(page.getByText('with-exif-gps.jpeg', { exact: true })).toBeVisible();
   30 | 		await expect(page.getByText(/\.jpeg$/)).toHaveCount(4);
   31 | 	});
   32 |
   33 | 	test('has the correct bounding boxes', async ({ page }) => {
   34 | 		/**
   35 | 		 *
   36 | 		 * @param {string} id
   37 | 		 * @param {number} count
   38 | 		 */
   39 | 		async function expectBoundingBoxesCount(id, count) {
   40 | 			await expect(
   41 | 				page.locator(`[data-id='${id}']`).getByTestId('card-observation-bounding-box')
   42 | 			).toHaveCount(count);
   43 | 		}
   44 |
   45 | 		await expectBoundingBoxesCount('000000', 1);
   46 | 		await expectBoundingBoxesCount('000001', 2);
   47 | 		await expectBoundingBoxesCount('000002', 1);
   48 | 		await expectBoundingBoxesCount('000003', 0);
   49 | 	});
   50 |
   51 | 	test('does not re-analyze when going to classify tab', async ({ page }) => {
   52 | 		await page.getByRole('link', { name: 'Classifier' }).click();
   53 | 		await page.getByText('cyan', { exact: true }).click({
   54 | 			timeout: 3_000
   55 | 		});
   56 | 		await expect(page.getByTestId('sidepanel')).toMatchAriaSnapshot(`
   57 | 		  - complementary:
   58 | 		    - img "Image 1 de l'observation cyan"
   59 | 		    - heading "cyan" [level=2]:
   60 | 		      - img
   61 | 		      - textbox "Nom de l'observation"
   62 | 		    - text: Espèce
   63 | 		    - combobox: Allacma fusca
   64 | 		    - code: 26%
   65 | 		    - button:
   66 | 		      - img
   67 | 		    - text: Alternatives
   68 | 		    - list:
   69 | 		      - listitem:
   70 | 		        - text: Sminthurus viridis
   71 | 		        - code: 12%
   72 | 		        - button:
   73 | 		          - img
   74 | 		      - listitem:
   75 | 		        - text: Caprainea marginata
   76 | 		        - code: 9%
   77 | 		        - button:
   78 | 		          - img
   79 | 		    - text: Genre
   ...
   94 | 		          - img
   95 | 		    - text: Famille
   96 | 		    - combobox: Sminthuridae
   97 | 		    - code: 17%
   98 | 		    - button:
   99 | 		      - img
  100 | 		    - text: Ordre
  101 | 		    - combobox: Symphypleona
  102 | 		    - code: 17%
  103 | 		    - button:
  104 | 		      - img
  105 | 		    - text: Date
  ...
  107 | 		    - button [disabled]:
  108 | 		      - img
  109 | 		    - paragraph: Moment où la photo a été prise
  110 | 		    - text: Localisation
  111 | 		    - textbox "Localisation"
  112 | 		    - button [disabled]:
  113 | 		      - img
  114 | 		    - paragraph: Endroit où la photo a été prise
  115 | 		    - text: Classe
  116 | 		    - combobox: Collembola
  117 | 		    - code: 17%
  118 | 		    - button:
  119 | 		      - img
  120 | 		    - text: Phylum
  121 | 		    - combobox: Arthropoda
  122 | 		    - code: 17%
  123 | 		    - button:
  124 | 		      - img
  125 | 		    - text: Règne
  126 | 		    - combobox: Animalia
  127 | 		    - code: 17%
  128 | 		    - button:
```
