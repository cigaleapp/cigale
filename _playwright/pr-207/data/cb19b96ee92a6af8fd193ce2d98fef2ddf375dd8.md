...
- Location: /home/runner/work/cigale/cigale/tests/cropper.spec.js:75:3
...
waiting for navigation until "load"
  navigated to "http://localhost:4173/#/crop/000001"
============================================================
    at /home/runner/work/cigale/cigale/tests/cropper.spec.js:77:15
```
...
index d8db04a..e1051ea 100644
--- a/src/lib/exif.js
+++ b/src/lib/exif.js
@@ -17,11 +17,11 @@ import { matches } from './utils.js';
 /**
  *
...
@@ -58,14 +58,21 @@ export async function processExifData(protocolId, imageId, imageBytes, file) {
 	});
...
@@ -52,12 +61,12 @@ if (import.meta.vitest) {
...
+	return tables.Image.state.some((img) => img.fileId === imageFileId);
 ...
 /**
@@ -73,7 +82,7 @@ export function imageIsClassified(image) {
  * @param {import('$lib/database.js').Image} image
  */
...
+	return Boolean(image.fileId || uiState.erroredImages.has(image.id));
 ...
 /**
@@ -93,54 +102,86 @@ export async function downloadImage(imageId, options) {
...
@@ -157,15 +198,16 @@ const MAXHEIGHT = ({ width, height }) => Math.round((MAXWIDTH * height) / width)
...
@@ -179,7 +221,51 @@ export async function resizeToMaxSize({ source }) {
...
index b7bcc0c..078d401 100644
--- a/src/lib/utils.js
+++ b/src/lib/utils.js
@@ -128,6 +128,19 @@ export function pick(subject, ...keys) {
...
+
  ...
  *
  * @param {*} str
@@ -164,6 +177,44 @@ export function clamp(value, min, max) {
...
diff --git a/src/routes/(app)/(sidepanel)/+layout.svelte b/src/routes/(app)/(sidepanel)/+layout.svelte
index 55f4bce..12a682c 100644
--- a/src/routes/(app)/(sidepanel)/+layout.svelte
+++ b/src/routes/(app)/(sidepanel)/+layout.svelte
@@ -1,7 +1,7 @@
 <script>
...
diff --git a/src/routes/(app)/(sidepanel)/PreviewSidePanel.svelte b/src/routes/(app)/(sidepanel)/PreviewSidePanel.svelte
index 12f5a37..8c4c0a2 100644
--- a/src/routes/(app)/(sidepanel)/PreviewSidePanel.svelte
+++ b/src/routes/(app)/(sidepanel)/PreviewSidePanel.svelte
@@ -1,6 +1,9 @@
 <script>
-	import { page } from '$app/state';
+	/**
...
diff --git a/src/routes/(app)/(sidepanel)/classify/+page.svelte b/src/routes/(app)/(sidepanel)/classify/+page.svelte
index 59eb272..4e31545 100644
--- a/src/routes/(app)/(sidepanel)/classify/+page.svelte
+++ b/src/routes/(app)/(sidepanel)/classify/+page.svelte
@@ -5,7 +5,7 @@
 	import * as db from '$lib/idb.svelte';
 	import { tables } from '$lib/idb.svelte';
 	import {
...
index b20ee82..fedaf20 100644
--- a/src/routes/(app)/(sidepanel)/import/+page.sve
```