...
@@ -127,7 +134,7 @@ export async function extractMetadata(buffer, extractionPlan) {
 			.filter(
 				([, extracted]) =>
 					extracted !== undefined &&
...
@@ -218,8 +225,8 @@ export function addExifMetadata(bytes, metadataDefs, metadataValues) {
 		if (value === undefined) continue;
 
 		if (
-			matches(def.infer, { latitude: { exif: 'string' }, longitude: { exif: 'string' } }) &&
-			matches(value, { latitude: 'number', longitude: 'number' })
+			type({ latitude: { exif: 'string' }, longitude: { exif: 'string' } }).allows(value) &&
+			type({ latitude: 'number', longitude: 'number' }).allows(def.infer)
 		) {
 			// XXX harcoded BS :/
