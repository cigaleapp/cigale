...
- Name: Cropper view >> autoskip enabled OR disabled >> go back to import view with escape key (autoskip off)
- Location: /home/runner/work/cigale/cigale/tests/cropper.spec.js:38:4
...
============================================================
    at /home/runner/work/cigale/cigale/tests/cropper.spec.js:42:16
```
...
```ts
   1 | import * as annotations from './annotations.js';
   2 | import { test, expect } from './fixtures.js';
   3 | import { importPhotos, setSettings, getSettings, setImageMetadata, getImage } from './utils.js';
   4 |
   5 | test.describe('Cropper view', () => {
   6 | 	test.beforeEach(async ({ page }) => {
   7 | 		await importPhotos({ page }, 'lil-fella', 'cyan', 'leaf');
   8 | 		await page.getByTestId('goto-crop').click();
   9 | 		await page.waitForURL((u) => u.hash === '#/crop/');
   10 | 	});
   11 |
   12 | 	test('should have all cards visible', async ({ page }) => {
   13 | 		await expect(page.getByText('lil-fella.jpeg', { exact: true })).toBeVisible();
   14 | 		await expect(page.getByText('cyan.jpeg', { exact: true })).toBeVisible();
   15 | 		await expect(page.getByText('leaf.jpeg', { exact: true })).toBeVisible();
   16 | 	});
   17 |
   18 | 	test.describe('autoskip enabled OR disabled', async () => {
...
+		/** Si les boîtes englobantes ont été analys��es. Pratique en particulier pour savoir s'il faut calculer les boîtes englobantes pour une image qui n'a aucune observation associée (chaque boudingbox crée une observation) */
...
+	});
 ...
 /**
@@ -151,9 +295,25 @@ export function matches(subject, pattern) {
  * @returns {-1|0|1}
  */
...
+
  ...
  * @param {number} value
  * @param {number} min
@@ -164,6 +324,88 @@ export function clamp(value, min, max) {
...
+		}
+
+		// We can assert the yield type, sinc
```