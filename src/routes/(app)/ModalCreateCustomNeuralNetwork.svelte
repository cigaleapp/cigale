<script lang="ts">
	...
	import ModelOutputShapeDiagram from '$routes/(app)/protocols/[id]/metadata/[metadata]/(todo)/ModelOutputShapeDiagram.svelte';
				...
				<Field composite label="Forme de la sortie">
					<section class="presets">
						<SegmentedGroup
							...
							bind:current={
								() =>
									entries(DETECTION_OUTPUT_SHAPE_PRESETS).find(
										([preset, shape]) => dequal(shape, d.output.shape)
									)?.[0] ?? 'custom',
								(preset) => {
