<script lang="ts">
	...
	let servers = $state<Array<{ domain: string; name?: string }>>([]);

	$effect(() => {
		void (async () => {
			if (!adding) return;
			servers = await adding.servers(databaseHandle());
		})();
	});
		...
		{#if adding?.auth === 'token'}
			...
			>
				{#snippet step_server({ done })}
					<EnumButtons
						options={servers.map(({ domain, name }) => ({
							key: domain,
							label: name ?? domain,
							subtext: name ? domain : '',
						}))}
