...
<Stepper
	steps={[
		...orEmpty(adding.servers && adding.servers.length > 1, {
				...
				<a
					...
					{@attach inAppBrowser({
						async onUrlChange(browser, url) {
							...
							const kobonaut = await browser.cookie('kobonaut').catch(() => null);
							...
							if (response.status >= 400) {
								await browser.close();
								toasts.error('Impossible de récupérer le token : échec de la requête');
							}
							...
							if (data instanceof ArkErrors) {
								await browser.close();
								toasts.error('Impossible de récupérer le token : réponse invalide');
