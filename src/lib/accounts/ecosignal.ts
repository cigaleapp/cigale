...
export default class Provider implements Account {
	...
	async *#items(protocol: DB.Protocol, session: DB.Session) {
		...
		while (!page || page.page_info.page < page.page_info.total_pages) {
			...
			for (const image of page.data) {
				...
				yield { action: 'Images', done, total: page.page_info.total };

				const gid = image.media_url.toString();

				const fileMetadata = fileMetadatas.find((m) =>
					image.labels.includes(m.ecosignal.label)
				);
				...
				if (fileMetadata) {
					...
					if (!file) {
						const response = await this.fetch('GET', image.media_url);
		...
		const obsMedia = media.filter(
			(m: Media): m is Media & { is: 'observation' } => m.is === 'observation'
		);
		const fileMedia = media.filter((m: Media): m is Media & { is: 'file' } => m.is === 'file');
	...
	async *upload(protocol: DB.Protocol, session: DB.Session) {
		...
		for (const filebatch of chunk(toUpload, 3)) {
			for await (const update of concurrently(filebatch, async function* (upload) {
		...
		for await (const { output } of concurrently(imageUploads, async function* (upload) {
	...
	async *#setMediaLabels({
		...
		for (const [label, ass] of assignments.entries()) {
			...
			const { data } = await this.json(
				...
				{
					label_id: labelId,
					media_ids: ass.map((a) => a.mediaId).filter(nonnull),
				}
			);
	...
	async #fromCigaleLabelId(protocol: DB.Protocol) {
		...
		return this.#resolveLabelId(name);
	}
	...
	#parseMetadataValueFileLabel(protocol: DB.Protocol, label: string) {
		const [base, bareMetadataId] = label.split('/');

		return {
			metadataId: resolveMetadataImportBare(protocol, bareMetadataId),
		};
	}
	...
	get #fastestPollInterval() {
		return Math.max(100, (1 / this.MAX_REQUESTS_PER_SECOND.read) * 1e3);
	}
