...
export default class Provider implements Account {
	...
	async *sessions({
		...
		for (const p of await this.db.getAll('Protocol')) {
			if (filters.protocol && p.id !== filters.protocol) continue;
			...
			const collections = await this.json(
				...
				Provider.CollectionsResponse,
				{
					...
					creator_id: filters.mine ? this.userId : undefined,
			...
			if (!file) {
				...
				try {
					if (preview) {
						...
						console.warn(
							`EcoSignal image ${image.media_url(this.domain)} has no associated preview, not creating ImagePreviewFile!`,
							image
						);
					}
				} catch (e) {
					console.warn(
						`EcoSignal preview ${preview.url(this.domain)} couldnt be downloaded`,
						e
					);
				}
			}
	...
	async *upload(protocol: DB.Protocol, session: DB.Session) {
		...
		const collectionId = await this.#setCollection(protocol, session);
	...
	async #setCollection(protocol: DB.Protocol, session: DB.Session) {
		...
		if (!collectionId) {
			const created = await this.json(
				...
				{
					name: session.name,
					description: this.#compoundMetadataToMultilineField(session.description, {
	...
	async *#setAnnotations({
		...
		for (const [i, image] of fileImages.entries()) {
			...
			const annotationData = {
				...
				comments: this.#compoundMetadataToMultilineField('', {
	...
	async #fileUploadStatus(batchId: string, filename: string) {
		const { data: status } = await this.json(
			'GET',
			'v1',
			`file-upload-batches/${batchId}/files/${encodeURIComponent(
				this.#conformFilename(filename)
			)}`,
	...
	#protocolEcosignalConfig(protocol: DB.Protocol) {
		...
		return {
			...
			domains,
			cropMetadataId:
				metadataId(config.crop) ??
				throwError(
					`No metadata with ${config.crop} found (Fix remote.ecosignal.crop in protocol definition file)`
				),
	...
	#resolveBoundingBox(
		...
		const { score, ...coords } = this.#boundingBoxResolver(0, [
			...
			ann.min_y,
			ann.max_y,
			ann.comments?.meta?.boxConfidence ?? 1,
		]);
	...
	async fetch(
		...
		console.debug(`[EcoSignal] ${method} ${pathname}`, {
			query: queryParams,
			...init,
		});
	...
	static MULTI_LINE_COMPOUND_FIELD_PREFIX = '--- Cigale metadata ---\n';
	static MULTI_LINE_COMPOUND_FIELD_SUFFIX = '\n--- End of cigale metadata ---';
		...
		return (
			...
			YAML.stringify(
				Provider.MetadataFieldMultilinePayload.in({
					...meta,
					metadata: transformObject(toMetadataRecord(meta.metadata), (key, value) => {
	...
	static MediasResponse = Provider.ResponsePaginated({
		...
		note: Provider.MetadataFieldMultiline,
		labels: 'string[]',
	...
	static AnnotationsAllResponse = Provider.ResponseBase([
		{
			...
			creation_date: 'string.date.parse',
			comments: Provider.MetadataFieldMultiline,
		},
		'[]',
	]);
