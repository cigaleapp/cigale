...
export function Server(procedures) {
	...
	instance.start = (self) => {
		...
		console.log('[SWARP Server] Starting message listener on', self);

		self.addEventListener('message', async (event) => {
			const { functionName, input } = PayloadSchema.assert(event.data);
			console.log('[SWARP Server] Running', functionName, 'with', input);
			...
			await implementation(input, async (progress) => postMessage({ functionName, progress }))
...
export function Client(procedures) {
	...
	for (const functionName of Object.keys(procedures)) {
		instance[functionName] = async (input, onProgress = () => {}) => {
			...
			console.log('[SWARP Client] Calling', functionName, 'with', input);
