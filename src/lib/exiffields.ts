 ...
 */
export const EXIF_FIELDS_DETAILS = [
	{
		tag: 0x000b,
		ifd: 'Image',
...
export type ExifFieldKey = (typeof EXIF_FIELDS_DETAILS)[number]['key'];

export const EXIF_FIELDS = toMapping([...EXIF_FIELDS_DETAILS]);

function toMapping<Fields extends (typeof EXIF_FIELDS_DETAILS)[number][]>(fields: Fields) {
	return fromEntries(fields.map((field) => [field.key, field.tag]));
}

/**
 * exif-parser uses different keys than the actual EXIF standard....
 * so we convert them lol
 */
export function exifParserKeyToRealKey(key: string): ExifFieldKey {
	const found = [
		...entries(EXIF_PARSER_FIELDS.exif).map((entry) => ['', ...entry] as const),
		...entries(EXIF_PARSER_FIELDS.gps).map((entry) => ['GPSInfo', ...entry] as const),
	].find(([, , k]) => k === key);

	if (!found) {
		throw new Error(`${key} is not a valid exif-parser key`);
	}

	const [category, tag] = found;

	const real = EXIF_FIELDS_DETAILS.find(
		(field) =>
			(category === 'GPSInfo' ? field.ifd === 'GPSInfo' : field.ifd !== 'GPSInfo') &&
			field.tag === Number.parseInt(tag)
	);

	if (!real) {
		throw new Error(`${tag} does not correspond to any EXIF field`);
	}

	if (real.key !== key) {
		console.debug(`Mapping exif-parser key ${key} to real exif key ${real.key}`, real);
	}
