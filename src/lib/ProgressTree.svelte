...
<script lang="ts">
	...
	interface Props {
		...
		// eslint-disable-next-line no-unused-vars
		finish: undefined | ((state: 'ok' | 'error' | 'canceled') => void);
	}
	...
	.tree {
		...
		.leaf {
			...
			@container (width < 600px) {
				...
				grid-template-areas: 'text counts' 'bar bar';

				&:has(.icon.success) .bar {
					opacity: 0;
				}
			}
			...
			@container (width >= 600px) {
				...
				grid-template-areas: 'text bar counts';
				max-width: 67ch;
