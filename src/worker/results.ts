...
import { compareBy, entries, nonnull, progressSplitter, sum, UTF8_BOM } from '$lib/utils';
import { createZipArchive } from '$lib/zip.js';
...
swarp.generateResultsExport(
	async ({ sessionId, include, cropPadding, jsonSchemaURL, format }, notify, { abortSignal }) => {
		...
		for (const { file, path } of exportedMetadataFiles) {
			...
			files[path] = {
				content: new Uint8Array(await accessBytes('MetadataValueFile', file)),
		...
		files[filepaths.metadata.json] = {
			content: stringifyWithToplevelOrdering(
				'json',
		...
		if (format === 'folder') {
			notify({
				...
				data: {
					...
					content: files[filepaths.metadata.json].content,
				},
			});
		}
		...
		if (format === 'folder') {
			notify({
				...
				data: {
					...
					content: files[filepaths.metadata.csv].content,
				},
			});
		}
...
swarp.previewResultsZip(async ({ sessionId, include }, _, { abortSignal }) => {
	...
	return {
		...
		'metadata.files': exportedMetadataFiles.map(({ path, file: { contentType } }) => ({
			path,
			contentType,
		})),
		'images.cropped':
